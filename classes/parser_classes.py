...
class Webscraper:
    ...
    def __init__(self, data=None, updated_url=None):
        ...
        """
        self.__url = 'https://www.finmarket.ru/currency/rates/?id=10148&pv=1&cur={}&bd={}&bm={}&by={}&ed={}&em={' \
                     '}&ey={}&x=48&y=13#archive'
        self.__data = data
        self.__updated_url = updated_url
    ...
    def parse_page(self, currency, start_date, end_date, currencies):
        ...
        dt2 = end_date.split('-')
        self.__updated_url = self.__url.format(currency, dt1[2], dt1[1], dt1[0], dt2[2], dt2[1], dt2[0])
        page = requests.get(self.__updated_url)
...
class Global_currencies:
    def __init__(self, data=None):
        ...
        :param data: - данные полученные со страницы
        """
        self.__url = 'https://www.iban.ru/currency-codes'
        self.__data = data
    ...
    def update_data_base(self):
        ...
        data_tuples = list(self.__data.itertuples(index=False, name=None))
        cursor.executemany(update_query, data_tuples)
        conn.commit()
        conn.close()
